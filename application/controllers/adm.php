			...
			for($i = 0 ; $i < count($kd) ; $i++){
				for($aa = 0 ; $aa < count($jml_kd_per_soal);$aa++)
				{
					if($kd[$i]['id_kd'] == $jml_kd_per_soal[$aa]['id_kd']){
						$jml_kd_per_soal[$i]['jml_kd'] = $kd[$i]['point_kd'];
...
//////MATA PELAJARAN/////////
	...
	///KOMPETENSI DASAR///
		...
		//return as json
		$jeson = array();
		$a['data'] = $this->db->query("SELECT kd.* FROM kd")->result();
		if ($uri3 == "det") {
			$a = $this->db->query("SELECT * FROM kd WHERE id_kd = '$uri4'")->row();
			j($a);
			exit();
		} else if ($uri3 == "simpan") {
			$ket 	= "";
			if ($p->id_kd != 0) {
				$this->db->query("UPDATE kd SET kd_ke = '".bersih($p,"kd_ke")."',
								nama = '".bersih($p,"nama")."',
								id_mapel = '".bersih($p,"id_mapel")."'
								WHERE id_kd = '".bersih($p,"id_kd")."'");
				$ket = "edit";
			} else {
				$ket = "tambah";
			...
			$this->db->query("DELETE FROM kd WHERE id_kd = '".$uri4."'");
			$ret_arr['status'] 	= "ok";
			$ret_arr['caption']	= "hapus sukses";
			j($ret_arr);
			exit();
	...
	public function m_soal() {
		...
		if ($a['sess_level'] == "guru") {
			...
			$a['p_kd'] = $this->db->query("SELECT a.id_kd, a.nama
											FROM kd a
											INNER JOIN tr_guru_mapel b ON a.id_mapel = b.id_mapel
											WHERE b.id_guru = '".$a['sess_konid']."'")->result_array();
		} else {
			...
			$a['p_kd'] = $this->db->query("SELECT a.id_kd, a.nama
											FROM kd a
											INNER JOIN tr_guru_mapel b ON a.id_mapel = b.id_mapel")->result_array();
		}
		...
		if ($uri3 == "det") {
			...
			$asa = $this->db->query("SELECT a.id_kd, a.nama
											FROM kd a
											INNER JOIN tr_guru_mapel b ON a.id_mapel = b.id_mapel where b.id_mapel='".$uri4."'")->result_array();
			j($asa);
		    exit;
		} else {
			$a['p']	= "m_soal";
	...
	public function m_soal_essay(){
		...
		if ($uri3 == "det") {
			$a = $this->db->query("SELECT * FROM m_soal_essay WHERE id = '$uri4' ORDER BY id DESC")->row();
			j($a);
			exit();
		} else if ($uri3 == "import") {
			$a['p']	= "f_soal_import";
		} else if ($uri3 == "hapus_gambar") {
			$nama_gambar = $this->db->query("SELECT file FROM m_soal_essay WHERE id = '".$uri5."'")->row();
			$this->db->query("UPDATE m_soal_essay SET file = '', tipe_file = '' WHERE id = '".$uri5."'");
			@unlink("./upload/gambar_soal/".$nama_gambar->file);
			redirect('adm/m_soal_essay/pilih_mapel/'.$uri4);
		} else if ($uri3 == "pilih_mapel") {
			if ($a['sess_level'] == "guru") {
				$a['data'] = $this->db->query("SELECT m_soal_essay.*, m_guru.nama AS nama_guru FROM m_soal_essay INNER JOIN m_guru ON m_soal_essay.id_guru = m_guru.id WHERE m_soal_essay.id_guru = '".$a['sess_konid']."' AND m_soal_essay.id_mapel = '$uri4' ORDER BY id DESC")->result();
			} else {
				$a['data'] = $this->db->query("SELECT m_soal_essay.*, m_guru.nama AS nama_guru FROM m_soal_essay INNER JOIN m_guru ON m_soal_essay.id_guru = m_guru.id WHERE m_soal_essay.id_mapel = '$uri4' ORDER BY id DESC")->result();
			}
			//echo $this->db->last_query();
			$a['p']	= "m_soal_essay";
		} else if ($uri3 == "simpan") {
			...
			$pdata = array(
				...
				"id_mapel"=>$p['id_mapel'],
				//"bobot"=>$p['bobot'],
				"soal"=>$p['soal'], //"jawaban"=>$p['jawaban'],
				
			);

			if ($__mode == "edit") {
				$this->db->where("id", $p['id']);
				$this->db->update("m_soal_essay", $pdata);
				$__id_soal = $p['id'];
			} else {
				$this->db->set($pdata);
				$this->db->insert("m_soal_essay", $pdata);
				$get_id_akhir = $this->db->query("SELECT MAX(id) maks FROM m_soal_essay LIMIT 1")->row_array();
				$__id_soal = $get_id_akhir['maks'];
			}
			...
			if (!empty($nama_file['gambar_soal'])) {
				...
				$this->db->where("id", $__id_soal);
				$this->db->update("m_soal_essay", $data_simpan);
			}
			...
			$teks_gagal = "";
			foreach ($gagal as $k => $v) {
				$arr_nama_file_upload = array("gambar_soal"=>"File Soal ");
				$teks_gagal .= $arr_nama_file_upload[$k].': '.$v.'<br>';
			}
			...
			redirect('adm/m_soal_essay/pilih_mapel/'.$p['id_mapel']);
			...
			if ($uri4 == 0) {
				$a['d'] = array("mode"=>"add","id"=>"0","id_guru"=>$id_guru,"id_mapel"=>"","file"=>"","soal"=>"","tgl_input"=>"");
			} else {
				$a['d'] = $this->db->query("SELECT m_soal_essay.*, 'edit' AS mode FROM m_soal_essay WHERE id = '$uri4'")->row_array();
			...
			$nama_gambar = $this->db->query("SELECT id_mapel, file, opsi_a, opsi_b, opsi_c, opsi_d, opsi_e FROM m_soal_essay WHERE id = '".$uri4."'")->row();
			$pc_opsi_a = explode("#####", $nama_gambar->opsi_a);
			$pc_opsi_b = explode("#####", $nama_gambar->opsi_b);
			$pc_opsi_c = explode("#####", $nama_gambar->opsi_c);
			$pc_opsi_d = explode("#####", $nama_gambar->opsi_d);
			$pc_opsi_e = explode("#####", $nama_gambar->opsi_e);
			$this->db->query("DELETE FROM m_soal_essay WHERE id = '".$uri4."'");
			@unlink("./upload/gambar_soal/".$nama_gambar->file);
			@unlink("./upload/gambar_soal/".$pc_opsi_a[0]);
			@unlink("./upload/gambar_soal/".$pc_opsi_b[0]);
			@unlink("./upload/gambar_soal/".$pc_opsi_c[0]);
			...
			redirect('adm/m_soal_essay/pilih_mapel/'.$nama_gambar->id_mapel);
		} else if ($uri3 == "cetak") {
			$html = "<link href='".base_url()."___/css/style_print.css' rel='stylesheet' media='' type='text/css'/>";
			if ($a['sess_level'] == "admin") {
				$data = $this->db->query("SELECT * FROM m_soal_essay")->result();
			} else {
				$data = $this->db->query("SELECT * FROM m_soal_essay WHERE id_guru = '".$a['sess_konid']."'")->result();
			}
			...
			if (!empty($data)) {
				...
				$data_ok[3] = "Jml dipakai : ".($total)."<br>Benar: ".$jml_salah.", Salah: ".$jml_salah."<br>Persentase benar : ".number_format($persen_benar)." %";
				$data_ok[4] = '<div class="btn-group">
				  <a href="'.base_url().'adm/m_soal_essay/edit/'.$d['id'].'" class="btn btn-info btn-xs"><i class="glyphicon glyphicon-pencil" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Edit</a>
				  <a href="'.base_url().'adm/m_soal/hapus_essay/'.$d['id'].'" class="btn btn-danger btn-xs"><i class="glyphicon glyphicon-remove" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Hapus</a>
				 ';
	...
	public function m_ujian() {
		...
		// Tabahkan data buat dropdownlist Kelas
		$kelas = $this->db->query("Select * from kelas")->result();

			$a['kelas']=obj_to_array($kelas,"id_kelas,nama_kelas");
	...
	public function h_ujian() {
		...
		if ($uri3 == "det") {
	        ...
	        $q_datanya = $this->db->query("
	        	SELECT a.id, b.nim, b.nama, a.nilai, a.jml_benar
				FROM tr_ikut_ujian a
				INNER JOIN m_siswa b ON a.id_user = b.id
				WHERE a.id_tes = '$uri4' 
	...
	public function ikuti_ujian() {
		...
		$a['data'] = $this->db->query("SELECT 
									...
									INNER JOIN m_mapel b ON a.id_mapel = b.id
									INNER JOIN m_guru c ON a.id_guru = c.id
									LEFT JOIN tr_ikut_ujian d ON CONCAT('".$a['sess_konid']."',a.id) = CONCAT(d.id_user,d.id_tes) where a.id_kelas in (select id_kelas from m_siswa where id = '".$a['sess_konid']."')
	...
	public function ikut_ujian() {
		...
		if ($uri3 == "simpan_satu") {
			...
			if ($cek_sdh_selesai < 1) {
				...
				//Buat ambil data essay
				$queryEssay = "Select * from m_soal_essay where id_mapel = '".$cek_detil_tes->id_mapel."' AND id_guru = '".$cek_detil_tes->id_guru."'";
				$ambilEssay = $this->db->query($queryEssay)->result();
				$htmlEssay .= "<div class='alert alert-success' style='font-size:18px' >";
				
				$no = 1;
				//$htmlEssay='<ol>';
				foreach($ambilEssay as $isi){
					//$htmlEssay .= "<li>";
					
					$htmlEssay .= $no.". ".strip_tags($isi->soal);
					if(strip_tags($isi->file)!=''){
						$htmlEssay .= "<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src='".base_url()."upload/gambar_soal/".strip_tags($isi->file)."' />";
					}
					$htmlEssay .= "<br>";	
					$no++;
				}
	...
	public function sudah_selesai_ujian() {
		...
		if (empty($q_nilai)) {
			...
			if ($this->config->item('tampil_nilai')) {
				...
				$a['data'] = "<div class='alert alert-danger'>Anda telah selesai mengikuti ujian ini pada : <strong style='font-size: 16px'>".tjs($q_nilai->tgl_selesai, "l")."</strong>. Nilai akan diumumkan oleh Guru yang bersangkutan.<br><b>Terimakasih :)</b></div>";
			}
		}
		$this->load->view('aaa', $a);
	}
